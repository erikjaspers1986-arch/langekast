...
var effectiveBetEl=document.getElementById("effective-bet");
var lineModePanel=document.getElementById("line-mode-panel");
var lineMultiplier=document.getElementById("line-multiplier");
...
function animateLineMode(){
  if(!lineModePanel||!lineMultiplier)return;
...
function update(){
  ...
  if(effectiveBetEl)effectiveBetEl.textContent=formatCredits(getEffectiveBet());
  if(lineMultiplier)lineMultiplier.textContent=fiveLines?"×5":"×1";
  if(lineModePanel)lineModePanel.classList.toggle("five",fiveLines);
